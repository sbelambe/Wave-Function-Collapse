...
class sceneName extends Phaser.Scene {
  ...
  create() {
    ...
    const wfc = new WaveFunctionCollapse(20, 15, null); // replace null with rules
    const tileGrid = wfc.collapse(); // Get the generated 2D tile grid
    ...
    map.createLayer(0, tileset, 100, 100); // Adjust X/Y offsets if needed
    ...
    // const dyn_tileset = dyn_map.addTilesetImage(
    //   this.tileset.name,
    //   this.tileset.key
    // );

    // note: layer is needed for display
    // const dyn_layer = dyn_map.createBlankLayer(
    //   "layer1",
    //   dyn_tileset,
    //   startX,
    //   startY + 400
    ...
    // Add a debug tile at (0, 0) to test tile indices
    // IMPORTANT: 17 columns and 12 rows
    // To find indice, tileIndex = row * numColumns + column

    // Land = 5
    const debugTileIndex = 5; // Change this to test different tile numbers
